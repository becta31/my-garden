...
def get_ai_advice(plants_info, weather):
    ...
    # Используем проверенный вами путь для Gemini 2.0 Flash
    ...
    # Логика повторов при перегрузке (Retry Logic)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'candidates' in result and result['candidates']:
                    ...
                    # Чистим текст от спецсимволов разметки
                    return text.replace('*', '').replace('_', '').replace('#', '')
                return "Агроном задумался и не выдал текст."
            
            elif response.status_code == 429:
                if attempt < 2:
                    time.sleep(12) # Ждем чуть дольше при перегрузке
                    continue
                return "Агроном отдыхает (лимиты превышены). Попробуй через 15 минут."
            
            elif response.status_code == 404:
                return "Ошибка 404: Модель не найдена. Проверь версию API."
