# send_tasks.py (Level 2: полуавтомат + антидубль)
import os
import json
import re
import ast
...
def _text_blob(p):
    """Собираем текст, который уже и так будет показан пользователю, чтобы не дублировать."""
    parts = []
...
def semi_auto_hint(p, month_idx):
    """
    Возвращает 0–2 коротких подсказки.
    ...
    if stage in ("dormant", "покой"):
        # гранат: напомнить только если это не написано
    ...
    # 2) Осмокот — только март, если не упомянут
    ...
    # 3) Акварин — рост (март–июнь), если не упомянут
    ...
    # 4) МКФ — только цветущим, если не упомянут
    ...
    # 5) Орхидеи — мягко, если не упомянуто
    if "орхиде" in name and stage in ("foliage", "листва", "рост") and month_idx in [2, 3, 4, 5, 6, 7]:
        if not _already_covered(blob, ["0.3", "0.5", "раз в 2", "2–3 недели"]):
            hints.append("💡 Орхидея: держи дозу мягкой (0.3–0.5 г/л) и редко (раз в 2–3 недели).")
