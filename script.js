...
function renderCollection() {
  ...
  const toWater = plantsData.filter(
    p => p.waterFreq === 1 || d % p.waterFreq === 0
  ).length;
  ...
  plantsData.forEach(p => {
    ...
    list.innerHTML += `
      <div class="plant-card">
        ...
        <div class="info-item">
          <b>📍 Место:</b> ${p.location || 'Не указано'}
        </div>

        <div class="info-item">
          <b>💧 Полив:</b>
          ${p.waterFreq === 1 ? 'ежедневно' : 'раз в ' + p.waterFreq + ' дн.'}
        </div>

        <div class="history-box">
          <b>Последнее:</b> ${lastLog.date} — ${lastLog.event}
        </div>
      </div>
    `;
  });
...
function renderYearView() {
  ...
  plantsData.forEach(p => {
    ...
    for (let m = 0; m < 12; m++) {
      ...
      if (p.feedMonths?.includes(m)) { icons += "💊"; isActive = true; }
      if (p.pruneMonths?.includes(m)) { icons += "✂️"; isActive = true; }
      if (p.repotMonths?.includes(m)) { icons += "🪴"; isActive = true; }

      row += `
        <td class="${isActive ? 'cell-active' : ''} ${m === currentMonth ? 'current-month-col' : ''}">
          ${icons}
        </td>
      `;
    }
...
// ===============================
// Helper: today feed logic (как в боте)
// ===============================
function hasFeedToday(p, monthIdx, day) {
  if (!p.feedMonths || !p.feedMonths.includes(monthIdx)) return false;

  return (
    (p.waterFreq && p.waterFreq > 1) ||
    day === 1 ||
    day === 15
  );
}
...
function updateCalendar() {
  ...
  plantsData.forEach(p => {
    ...
    const freqText = p.waterFreq === 1
      ? 'ежедневно'
      : `раз в ${p.waterFreq} дн.`;
